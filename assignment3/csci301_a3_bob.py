...
def createBlock(block_num, transaction):
	...
	prev_block_f = open("b_" + str(block_num-1)+".json", "r")
	prev_block = prev_block_f.read()
	prev_block_f.close()
	prev_hash = hashlib.sha256(prev_block.encode()).hexdigest()

	# Start nonce at 1 000 000 000 to allow different hash
	nonce = 1000000000
...
def verifyBlock(current_block, current_block_num):
	...
	prev_block_f = open("b_" + str(current_block_num-1)+".json", "r")
	prev_block = prev_block_f.read()
	prev_block_f.close()
	prev_hash = hashlib.sha256(prev_block.encode()).hexdigest()
	...
	if int(prev_hash, 16) == int(hash_val, 16):
		print ("b_" + str(current_block_num - 1) + ".json is verified")
		verified = True
	else:
		print ("b_" + str(current_block_num - 1) + ".json is NOT verified")
...
for transaction in transactions:
	...
	fw = open("b_" + str(block_num)+".json", "w+")
	fw.write(block)
	fw.close()
	print ("b_" + str(block_num)+".json created.")
